...
업로드할 때는 HTTP 캐시를 사용하지 않고 최신 SHA와 파일을 읽습니다. 한 브라우저에서 들어온 업로드 요청은 순서대로 처리하며, 충돌이 발생하면 지수 지연과 짧은 무작위 지연을 두어 최대 4회 재시도합니다. 재시도에 실패하면 `chrome.storage.local`의 임시 기록을 삭제하지 않습니다.

popup의 **대기 기록 업로드**는 오래된 기록부터 모두 처리합니다. 이미 GitHub에 등록된 기록은 새 커밋을 만들지 않고 로컬 대기열에서 정리하며, 도중에 실패하면 완료된 기록만 제거하고 실패한 기록과 나머지는 유지합니다.
